...
//unsigned int time_t_keep = 1;
//unsigned int record_ctl = 1;
...
static void *encode_thread(void *arg)
{
	...
	//char file_path_sub[32] = "/mnt/sd/dump_bs_sub.jpg";
	FILE *f_out_sub = NULL;
		...
		if (HD_OK == hd_videoenc_poll_list(poll_list, poll_num, -1)) {    // multi path poll_list , -1 = blocking mode
			if (g_mainen) {
				if (TRUE == poll_list[main_poll_id].revent.event) {
					if (snapshot_count==1) 
					{
						...
						char file_path_main_tmp_tmp[60] = {0};
						
						if (file_create==1)
						{
							char filename[40];
    						JudgeDoVideo(filename);

							snprintf(file_path_main_tmp_tmp, 60, "/mnt/sd/%s", filename);
							printf("dump snapshot bitstream to file (%s) ....\r\n", file_path_main_tmp_tmp);

							//----- open output files -----
							if ((f_out_main = fopen(file_path_main_tmp_tmp, "wb")) == NULL)
								HD_VIDEOENC_ERR("open file (%s) fail....\r\n\r\n", file_path_main_tmp_tmp);
						...
						if (ret == HD_OK) {
							...
							//fclose(f_out_main);
							//if ((f_out_main = fopen(file_path_main_tmp_tmp, "r")) == NULL)
							//	HD_VIDEOENC_ERR("open file (%s) fail....\r\n\r\n", file_path_main_tmp_tmp);
							char buffer_f[data_pull.video_pack[j].size*data_pull.pack_num];
							FILE *my_picture;
							if ((my_picture = fopen("/mnt/sd/dump_bs_snapeoxxxx.jpg", "wb")) == NULL)
								HD_VIDEOENC_ERR("open file (%s) fail....\r\n\r\n", file_path_main_tmp_tmp);
							printf("open ess\n");
							int f_size = fread(buffer_f, 1, data_pull.video_pack[j].size*data_pull.pack_num, f_out_main);
							fwrite(buffer_f, 1, data_pull.video_pack[j].size*data_pull.pack_num, my_picture);
							//fclose(my_picture);
							if (f_size) printf("open success\n");}
							//fclose(f_out_main);
					...
					{
						ret = hd_videoenc_pull_out_buf(p_stream0->enc_path, &data_pull, 0);
			...
			if (g_snapshoten) {
				if (TRUE == poll_list[snap_poll_id].revent.event) {
					...
					printf("snapshoten enter\n");

					if (snapshot_count%50==0) 
					{   
						char file_path_main_tmp_tmp[32] = {0};    
						snprintf(file_path_main_tmp_tmp, 32, "/mnt/sd/dump_bs_x%d.jpg", snapshot_count);
						printf("dump snapshot bitstream to file (%s) ....\r\n", file_path_main_tmp_tmp);
			...
			if (g_suben) {
				...
				if (FALSE) {
					...
					if (snapshot_count%300==0) {   
						char file_path_main_tmp_tmp[32] = {0};    
						snprintf(file_path_main_tmp_tmp, 32, "/mnt/sd/picture%d.dat", snapshot_count);
						printf("dump snapshot bitstream to file (%s) ....\r\n", file_path_main_tmp_tmp);

						//----- open output files -----
						if ((f_out_sub = fopen(file_path_main_tmp_tmp, "wb")) == NULL)
							HD_VIDEOENC_ERR("open file (%s) fail....\r\n\r\n", file_path_main_tmp_tmp);
					}
...
static void *snapshot_thread(void *arg)
{
	...
	printf("\r\nxxxxxxxxxxxxxxxxxxxxxxx \"s\" to trigger !!\r\n");
	//------ wait flow_start ------
	while (p_stream0->enc_enter == 0) sleep(1);

	// query physical address of bs buffer ( this can ONLY query after hd_videoenc_start() is called !! )
	hd_videoenc_get(p_stream2->enc_path, HD_VIDEOENC_PARAM_BUFINFO, &phy_buf_main);
	...
	for (int i=0; i<20; i++)
	{
			...
			snprintf(file_path_main, 32, "/mnt/sd/dump_bs_snap%u.jpg", i);
			printf("dump snapshot bitstream to file (%s) ....\r\n", file_path_main);
...
static void *time_thread(void *arg)
{
	...
	while (time_t_keep == 1)
	{
		if (record_ctl == 1)
		{
			...
			{
				t1.tv_sec = get_CamPIR_int("videotimes");
			}
			
			printf("record time t1.tv_sec:%d", t1.tv_sec);
			int err;
			do {
			...
			char exec_string[200] = {0};
			char videoname[40] = {0};
			char picturename[40] = {0};
			GetLastFilename(videoname);
			strcpy(picturename, videoname);

			{
				strcpy(picturename+strlen(picturename)-4, ".jpg");
			}

			//strcat(picturename, ".jpg");
			snprintf(exec_string, 200, "ffmpeg -i /mnt/sd/%s  -y -f image2 -ss 0 -s 1280x720 /mnt/sd/%s", videoname, picturename);
			system(exec_string);
			
			
			printf("\n###########\nvideofile:%s\npicturefile:%s\n", videoname, picturename);

			if (get_CamPIR_int("mode")==2)
			{
				FileListDelet();
				char videofilepath[60] = {0};
				snprintf(videofilepath, 60, "/mnt/sd/%s", videoname);
				remove(videofilepath);
				FileListAdd(picturename);
			}
			else if (get_CamPIR_int("mode")==3)
			{
				char picturefilepath[60] = {0};
				snprintf(picturefilepath, 60, "/mnt/sd/%s", picturename);
				remove(picturefilepath);
			}
			else if (get_CamPIR_int("mode")==1)
			{
				FileListAdd(picturename);
			}

			record_ctl_file_ok = 1;
	...
	printf("time thread out\n");

	return 0;
}

void VideoCaptureMain(void)
{
	HD_RESULT ret;
	//INT key;
...
/*
	...
	ret = pthread_create(&snap_thread, NULL, snapshot_thread, (void *)stream);
	if (ret < 0) {
		printf("create snapshot_thread failed");
		goto exit;
	}
*/
	...
	ret = pthread_create(&time_thread_t, NULL, time_thread, (void *)stream);
	if (ret < 0) {
		printf("create time_thread failed");
		goto exit;
	}
	...
	pthread_join(time_thread_t, NULL);
...
{
	pthread_t main_t;
	pthread_create(&main_t, NULL, (void*)VideoCaptureMain, NULL);
